...
## Épilogue du Chapitre III : le Reflet vaincu, et l'amorce du Chapitre IV.
func _show_chapter_recap(results: Dictionary) -> void:
	...
	var title := Label.new()
	title.text = "Le Reflet est brisé !"
	...
	epilogue.add_theme_color_override("font_color", Color(0.9, 0.92, 0.96))
	box.add_child(epilogue)

	box.add_child(_spacer(6.0))

	var run := Label.new()
	run.text = "Le Reflet vaincu — Grade : %s — %s — Orbes : %d/%d — Esprits vaincus : %d" % [
	...
	header.text = "Ton périple, Chapitres I à III :"
	...
	var replay := _recap_button("Rejouer le combat", Color(0.5, 0.8, 1.0))
	...
	suite.add_theme_color_override("font_color", Color(0.6, 0.85, 1.0))
	box.add_child(suite)
	var chap := Label.new()
	chap.text = "Chapitre IV — Au-delà du Miroir"
	chap.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	chap.add_theme_font_size_override("font_size", 18)
	chap.add_theme_color_override("font_color", Color(0.85, 0.9, 0.95))
	box.add_child(chap)
	var hook := Label.new()
	hook.text = "Derrière le tain brisé s'ouvre un lieu que nul reflet ne renvoie : la source de toutes les images. Ce qui se cachait derrière le miroir n'a plus de visage à emprunter — et il a vu Eneko. La Voie du Sabre mène désormais au-delà du verre."
	...
	hook.add_theme_color_override("font_color", Color(0.82, 0.85, 0.9))
	box.add_child(hook)
	box.add_child(_spacer(10.0))
...
func _recap_button(label_text: String, accent: Color) -> Button:
	...
	sb.bg_color = Color(0.1, 0.11, 0.17, 0.92)
	sb.border_color = accent
	sb.set_border_width_all(2)
	sb.set_corner_radius_all(10)
	sb.set_content_margin_all(8.0)
	var hov: StyleBoxFlat = sb.duplicate()
	hov.bg_color = Color(0.16, 0.2, 0.28, 0.95)
	var prs: StyleBoxFlat = sb.duplicate()
	prs.bg_color = Color(0.2, 0.28, 0.36, 0.95)
