...
## Répliques par défaut (première rencontre, niveau 1). Un niveau peut les
## remplacer avant le début de partie via set_lines().
const DEFAULT_LINES := [
...
## Remplace les répliques par défaut (à appeler juste après instanciation,
## avant que le joueur ne puisse l'approcher).
